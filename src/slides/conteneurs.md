...
Cette approche permet à docker d'**optimiser le téléchargement et la construction des images avec des mécanismes de cache**.
...
Nous utiliserons principalement les deux exécutables suivant pour qui sont des **clients en ligne de commande pour l'API docker** :
...
Nous noterons qu'il est possible de stocker des images avec des outils tels [Harbor](https://goharbor.io/) ou [Nexus3](https://hub.docker.com/r/sonatype/nexus3) déployés en propre.
...
Nous trouverons aussi des dépôts d'image au niveau des solutions d'hébergement (ex : Google Container Registry).
...
A l'instar d'Ansible, il faudrait de nombreuses séances pour vous former réellement à l'utilisation de docker. Nous allons nous contenter d'une découverte des principaux concepts à l'aide d'exemples.
...
* [mborne/docker-devbox](https://github.com/mborne/docker-devbox/#mbornedocker-devbox) qui est mon terrain de jeu pour docker et Kubernetes
...
* Vous pourrez assez facilement **installer docker sur une VM Linux**
...
> Pour l'installation, il faudra principalement traiter la problématique du [proxy sortant](annexe/proxy-docker.html) et éventuellement la résolution DNS.
...
**Éviter d'installer directement le démon docker directement sur un poste de travail branché sur le réseau de votre école/entreprise**. Il faut :

* Éviter les conflits d'IP entre docker et le réseau local.
* [Définir les bonnes options de sécurité](https://github.com/docker/docker-bench-security#docker-bench-for-security)
* Éventuellement faire cohabiter docker avec un VPN
* ...

> Les commandes `docker network ls` et `ip link` vous permettrons de constater que docker créé des réseaux virtuels.
...
Pour la prise en main de l'exécutable `docker` et la découverte des concepts, nous allons voir ensemble les exemples stockés dans un dépôt dédié annexé à ce cours :
