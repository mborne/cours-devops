...
**La gestion des infrastructures est un sujet sensible**. Pour faire évoluer les pratiques et les processus, il faudra d'abord une compréhension partagée :
...
Pour faire simple, avant de cibler une **infrastructure agile**, il faudra être nombreux à voir qu'il y a un problème quand :

* Il faut des jours pour re-livrer une application avec une mise à jour de ses dépendances (cas récent : [faille Log4Shell](https://fr.wikipedia.org/wiki/Log4Shell))
* Il faut croiser les doigts pour que les procédures de déploiement soit à jour.
...
Le calcul de ces métriques sera délicat sans :
...
La mise en oeuvre sera triviale avec des outils tels [UptimeRobot](https://uptimerobot.com/). Les outils de supervision pourront généralement être étendus pour ajouter ce type de sonde (ex : [prometheus/blackbox_exporter](https://github.com/prometheus/blackbox_exporter#blackbox-exporter-)).
...
* Un document est extrait d'une solution bureautique, édité sur un poste et renvoyé par mail
...
> Il existe des outils pour détecter les secrets dans les dépôts, par exemple [git-secrets](https://github.com/awslabs/git-secrets#git-secrets). Il est conseillé de les utiliser avant la concurrence...
