...
> Nous noterons l'existence d'une alternative à [fail2ban](https://doc.ubuntu-fr.org/fail2ban) : [CrowdSec](https://github.com/crowdsecurity/crowdsec#readme) où les IP suspectes sont partagées. On s'assurera de respecter le [RGPD](https://www.cnil.fr/fr/rgpd-de-quoi-parle-t-on) avec ce type d'outil.
...
Nous noterons au passage que prévoir un traitement particulier pour les **services de stockage** simplifiera la mise en oeuvre d'un **plan de sauvegarde** et d'un **plan de reprise d'activité (PRA)**.
...
Nous soulignerons qu'il sera difficile possible de distinguer :
...
* Les conteneurs solutionnent ce problème via l'utilisation d'image embarquant les dépendances des applications.
* Kubernetes va un cran plus loin en permettant aux équipes produit de définir par exemple la configuration du LoadBalancer avec le concept [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/#what-is-ingress) plutôt que via un ticket ou un document.
