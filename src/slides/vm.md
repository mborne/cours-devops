...
### Le délicat recours à un prestataire...
...
* Avec "je veux des mises à jour régulière", le prestataire ajoutera potentiellement en bonus un `rm -rf /etc/apt/sources.list.d/*` pour reconfigurer ses seuls dépôts et vous aurez de belles surprises!
* Avec "je veux une exécution régulière de `apt-get update && apt-get upgrade -y`, il y aura moins de place pour la fantaisie.

> (Du moins, ce type de problématique nous aidera à comprendre l'intérêt des conteneurs et de l'orchestration de conteneur...)
